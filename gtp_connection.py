...
class GtpConnection:
    def __init__(self, go_engine, board, debug_mode=False):
        ...
        self.argmap = {
            ...
            "timeLim": (1, 'Usage: timelimit SECONDS')
        }
    ...
    def get_move(self, color, timelimit):
        wincheck, move = self.solve(self.board, self.timelimit)
        if move is not None:
            return move
    ...
    def genmove_cmd(self, args):
        ...
        board_color = args[0].lower()
        color = color_to_int(board_color)
        move = self.get_move(color,self.timelimit)
    ...
    def timelimit_cmd(self, args):
        temp = args[0]
        if  temp.isdigit() and 100 >= int(temp) >= 1:
            self.timelimit = int(temp)
            self.respond()
        else:
            self.respond("wrong input format")
    ...
    def solve(self, board, timelimit):   
        ...
        legalList = GoBoardUtil.generate_legal_moves(board_copy,board_copy.current_player)
        if len(legalList) == 1:
            board_copy.play_move(legalList[0],board_copy.current_player)
            result = board_copy.detect_five_in_a_row()
            if result == EMPTY:
                return "draw", legalList[0]
            elif result == BLACK:
                return 'b', legalList[0]
            elif result == WHITE:
                return 'w', legalList[0]
        signal.signal(signal.SIGALRM, self.handler)
        signal.alarm(self.timelimit)
        try:
            score, move = callalphabeta(board)
            #print(score)
            if score == 0:
                return "draw", move
            else:
